...
#represents the individual file the loop willl iterate through
...
for file in os.listdir('Originals/'):
    ...
    crppd_img_YCb = YCbCrskinMask(image)
    crppd_img_HSV = HSVskinMask(image)

    hsvCrp.append(crppd_img_HSV)
    ycbcrCrp.append(crppd_img_YCb)

    base_name = file.split(".")[0]
    #cv2.imwrite(f'CrppdImg_HSV/{base_name}_C.png',crppd_img_HSV)
    #cv2.imwrite(f'CrppdImg_YCrCb/{base_name}_C.png',crppd_img_YCb)

for imgs in hsvCrp:
    compPlot[0,0].set_title('HSV')
    compPlot[0,0].imshow(hsvCrp[0])
    compPlot[0,1].set_title('YCbCr')
    compPlot[0,1].imshow(ycbcrCrp[0])
